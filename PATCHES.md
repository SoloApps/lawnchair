...
| 7 | `src/com/android/launcher3/touch/ItemClickHandler.java` | Klik op vergrendelde private app | Twee gemarkeerde plekken: (a) in `startAppShortcutOrInfoActivity` wordt voor een vergrendelde private app (`isPrivateProfileLocked`) eenmalig `requestUnlockThenRun(...)` aangeroepen; na ontgrendelen komt de methode opnieuw langs en start de app direct. (b) in `onClickAppShortcut` wordt de disabled-afhandeling (`handleDisabledItemClicked`) overgeslagen voor een vergrendelde private app, anders vangt Android's `FLAG_DISABLED_LOCKED_USER` (gezet bij verse boot) de klik af met een "niet beschikbaar"-toast en wordt de unlock-op-tik nooit bereikt. |
